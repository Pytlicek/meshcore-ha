...
async def async_setup_services(hass: HomeAssistant) -> None:
            ...
            if api and api.connected:
                try:
                    ...
                    if node_id is not None:
                        ...
                        if not contact:
                            _LOGGER.error(
                                f"Contact with pubkey prefix '{pubkey_prefix}' not found"
                            )
                            continue
                    ...
                    if result.type == EventType.ERROR:
                        ...
                        display_name = (
                            contact_name if contact_name else target_identifier
                        )
                        pubkey = contact.get("public_key", "Unknown")
                        _LOGGER.info(
                            "Successfully sent message to %s, pubkey: %s",
                            display_name,
                            pubkey,
                        )
    ...
    async def async_send_channel_message_service(call: ServiceCall) -> None:
        ...
        for config_entry_id, coordinator in hass.data[DOMAIN].items():
            ...
            _LOGGER.debug(
                "Entry ID: %s, coordinator: %s", config_entry_id, coordinator.name
            )
            # If entry_id is specified, only use the matching entry
            if entry_id and entry_id != config_entry_id:
                continue
            ...
            if api and api.connected:
                try:
                    ...
                    if result.type == EventType.ERROR:
                        _LOGGER.warning(
                            "Failed to send message to channel %s: %s",
                            channel_idx,
                            result.payload,
                        )
                    else:
    ...
    async def async_message_script_service(call: ServiceCall) -> None:
        ...
        if not recipient_type:
            _LOGGER.error(
                "Recipient type helper not found: select.meshcore_recipient_type"
            )
            return
    ...
    async def async_execute_command_service(call: ServiceCall) -> None:
        ...
        _LOGGER.debug(
            "Executing command: %s with arguments: %s", command_name, arguments
        )
            ...
            if api and api.connected:
                try:
                    ...
                    command_param_types = {
                        ...
                        "get_contacts": [
                            "int"
                        ],  # lastmod parameter (optional, defaults to 0)
                        "reset_path": ["contact"],
                        ...
                        "send_msg": [
                            ...
                            "str",
                            "int",
                        ],  # contact, message, timestamp (optional)
                        "send_chan_msg": [
                            "int",
                            "str",
                            "int",
                        ...
                        "send_cmd": [
                            ...
                            "str",
                            "int",
                        ],  # contact, command, timestamp (optional)
                        ...
                        "send_trace": [
                            ...
                            "int",
                            "bytes",
                        ],  # hop_count, timestamp, flags, data
                        # Binary commands
                        "req_binary": [
                            "contact",
                            "str",
                    ...
                    for i, arg in enumerate(arguments):
                        ...
                        if param_type == "contact":
                                    ...
                                    if contact:
                                        ...
                                        if command_name == "add_contact":
                                            for (
                                                ...
                                                coordinator._discovered_contacts.values()
                                            ):
                                                if (
                                                    discovered_contact.get(
                                                        "public_key", ""
                                                    ).startswith(arg)
                                                    or discovered_contact.get(
                                                        "adv_name"
                                                    )
                                                    == arg
                                                ):
                                        ...
                                        if not contact:
                                            _LOGGER.error(
                                                f"Contact not found with key or name: {arg}"
                                            )
                                            return
                            else:
                            ...
                            try:
                                ...
                                _LOGGER.error(
                                    f"Could not convert '{arg}' to bytes - invalid hex string"
                                )
                                return
                    ...
                    _LOGGER.debug(
                        f"Executing {command_name} with prepared arguments: {prepared_args}"
                    )
                    ...
                    if command_name == "set_channel" and result.type != EventType.ERROR:
                        ...
                        channel_info_result = await api.mesh_core.commands.get_channel(
                            channel_idx
                        )
                        if channel_info_result.type != EventType.ERROR:
                            coordinator._channel_info[channel_idx] = (
                                channel_info_result.payload
                            )
                            _LOGGER.info(
                                f"Updated channel {channel_idx} info: {channel_info_result.payload}"
                            )
                    ...
                    if command_name == "add_contact" and result.type != EventType.ERROR:
                        ...
                        if contact_to_add and isinstance(contact_to_add, dict):
                            ...
                            if pubkey:
                                ...
                                updated_data["contacts"] = (
                                    coordinator.get_all_contacts()
                                )
                                coordinator.async_set_updated_data(updated_data)
                    elif (
                        command_name == "remove_contact"
    ...
    async def async_execute_command_ui_service(call: ServiceCall) -> None:
        ...
        command_call = create_service_call(
            DOMAIN, SERVICE_EXECUTE_COMMAND, {"command": command, "entry_id": entry_id}
        )
    ...
    async def async_add_selected_contact_service(call: ServiceCall) -> None:
        ...
        if entry_id:
            ...
            for state in hass.states.async_all():
                if (
                    state.entity_id.startswith("select.")
                    and "discovered_contact" in state.entity_id
                ):
        ...
        if not selected_option or selected_option in [
            SELECT_NO_CONTACTS,
            SELECT_NO_DISCOVERED,
        ]:
    ...
    async def async_remove_all_discovered_contacts_service(call: ServiceCall) -> None:
        """Odstráni všetky objavené kontakty pre daný entry_id alebo všetky entry_id."""
        entry_id = call.data.get(ATTR_ENTRY_ID)

        # Získame zoznam coordinatorov, ktoré ideme spracovať
        coordinators_to_process = []
        if entry_id:
            if entry_id in hass.data[DOMAIN]:
                coordinators_to_process = [hass.data[DOMAIN][entry_id]]
        else:
            # Ak entry_id nie je zadané, vezmeme všetkých coordinatorov
            coordinators_to_process = [
                coord
                for coord in hass.data[DOMAIN].values()
                if hasattr(coord, "_discovered_contacts")
            ]

        for coordinator in coordinators_to_process:
            # Získame prefixy všetkých objavených kontaktov
            prefixes = list(coordinator._discovered_contacts.keys())

            if not prefixes:
                _LOGGER.info("Žiadne objavené kontakty na vymazanie pre tento uzol.")
                continue

            _LOGGER.info(f"Mažem {len(prefixes)} objavených kontaktov.")

            for prefix in prefixes:
                # Tu voláme priamo internú logiku odstránenia z pamäte coordinatora
                if prefix in coordinator._discovered_contacts:
                    del coordinator._discovered_contacts[prefix]

            # Oznámime systému, že sa dáta zmenili, aby sa aktualizovali UI prvky (select boxy)
            updated_data = dict(coordinator.data) if coordinator.data else {}
            updated_data["discovered_contacts"] = coordinator._discovered_contacts
            coordinator.async_set_updated_data(updated_data)

        _LOGGER.info("Hromadné čistenie objavených kontaktov dokončené.")
    ...
    async def async_remove_selected_contact_service(call: ServiceCall) -> None:
        ...
        if entry_id:
            ...
            for state in hass.states.async_all():
                if (
                    state.entity_id.startswith("select.")
                    and "added_contact" in state.entity_id
                ):
                    select_entity_id = state.entity_id
                    break
    ...
    async def async_remove_discovered_contact_service(call: ServiceCall) -> None:
        ...
        if not pubkey_prefix:
            ...
            if not select_entity_id:
                _LOGGER.error(
                    "Discovered contact select entity not found and no pubkey_prefix provided"
                )
                return
            ...
            if not pubkey_prefix:
                _LOGGER.error(
                    f"Could not parse pubkey from selection: {selected_option}"
                )
                return
        ...
        contact_name = coordinator._discovered_contacts[full_pubkey].get(
            "adv_name", "Unknown"
        )
    ...
    hass.services.async_register(
        DOMAIN,
        "remove_all_discovered_contacts",
        async_remove_all_discovered_contacts_service,
        schema=UI_MESSAGE_SCHEMA,
    )
...
def create_service_call(
    ...
    if MAJOR_VERSION >= 2025:
        ...
        return ServiceCall(hass=hass, domain=domain, service=service, data=data or {})
    else:
        _LOGGER.debug("Creating ServiceCall without hass parameter (2024.x.x format)")
        return ServiceCall(domain=domain, service=service, data=data or {})
